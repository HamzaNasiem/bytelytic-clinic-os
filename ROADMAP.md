...
- [x] `dashboard/src/pages/Signup.jsx` — multi-step wizard (4 steps)
- [x] Step 1: Clinic info (name, specialty, city, timezone)
- [x] Step 2: Doctor info (name, credentials, contact)
- [x] Step 3: Business hours (Mon-Fri time pickers)
- [x] Step 4: Appointment types (name + duration, add/remove)
- [x] Submit → triggers full auto-provisioning flow API
...
# PHASE 9 — Voice Cloning & Customization

**Status: 🔲 NOT STARTED**
**Goal:** Make the AI sound exactly like the clinic's real receptionist and customize its behavior

---

### 1. ElevenLabs Voice Cloning

- [ ] `src/services/voice_cloning.service.js` — ElevenLabs API integration
- [ ] Dashboard UI: "Upload 2-minute audio of your receptionist"
- [ ] API: `POST /clinics/:id/clone-voice` — sends MP3 to ElevenLabs, returns `voice_id`
- [ ] DB Schema: add `custom_voice_id` to clinics table
- [ ] `src/services/voice.service.js` — when creating Retell agent, use the `custom_voice_id` if available
- [ ] Fallback: Standard curated voices (Male/Female/British) if custom not provided

### 2. Custom Call Routing (Human Handoff)

- [ ] Schema: add `forwarding_number` and `routing_rules` to clinics table
- [ ] AI prompt logic: "If patient says emergency, say 'Transferring you now' and trigger `action: 'transfer'`"
- [ ] `src/services/voice.service.js` — intercept `action: 'transfer'` from Retell LLM response
- [ ] Twilio API: initiate call bridge to the clinic's `forwarding_number`
- [ ] Dashboard UI: Settings → Routing tab (Set up emergency number)

### 3. Smart Voicemail Drop

- [ ] Retell agent prompt update: Detect answering machine vs human
- [ ] If answering machine → wait for beep → leave customized voicemail message
- [ ] Outcome logged as `voicemail_left` in `calls` table

---

---

# PHASE 10 — Multi-Channel Omnipresence

**Status: 🔲 NOT STARTED**
**Goal:** Be available everywhere the patient is (WhatsApp, Web, Outbound)

---

### 1. WhatsApp Business Integration

- [ ] Twilio WhatsApp Sandbox / Business API integration
- [ ] `src/api/webhooks/twilio.webhook.js` — support incoming WhatsApp messages
- [ ] AI Service update: allow text-based booking conversation without voice
- [ ] Message history unified in `sms_messages` table (flagged as `whatsapp`)
- [ ] Dashboard: "Connect WhatsApp" in Settings

### 2. Website Chat Widget

- [ ] `dashboard/public/widget.js` — embeddable JS script for clinic websites
- [ ] Socket.io connection to backend for real-time chat
- [ ] UI: Bytelytic branded chat bubble in bottom-right corner
- [ ] AI uses exact same prompt/booking logic as the phone agent

### 3. Outbound Marketing AI Blasts

- [ ] `src/services/marketing.service.js`
- [ ] Dashboard UI: "Campaigns" tab
- [ ] Action: Select multiple patients → type custom prompt: "We have an opening today at 3pm, reply YES to grab it"
- [ ] System sends SMS blast via Twilio
- [ ] AI automatically handles all replies, first person to say "YES" gets booked, others get "Sorry, it's taken!"
- [ ] Track campaign ROI in `revenue_events`
...
| 9 | Voice Cloning & Customization | 🔲 Not Started | ElevenLabs voice clones, Human Call Transfers |
| 10 | Multi-Channel Omnipresence | 🔲 Not Started | WhatsApp booking, Chat widget, SMS Blasts |
...
| Custom Voice Cloning | Seamless brand experience | Higher patient trust |
| WhatsApp / Web Chat | Capture leads everywhere | +20% overall bookings |
